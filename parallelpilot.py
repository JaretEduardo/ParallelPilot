...
def get_codex_client(repo_dir: Path, model: str):
    try:
        return CodexSDKClient(repo_dir, model=model)
    except ImportError:
        console.print("[yellow]openai-codex not installed — using subprocess mode.[/yellow]")
        return CodexExecClient(repo_dir, model=model)


@app.command()
def main(
    script: str = typer.Argument(..., help="Script serial a paralelizar"),
    ...
    if not parallel_ok:
        console.print(f"\n[red]Could not obtain a correct version after {MAX_RETRIES} attempts.[/red]"); raise typer.Exit(1)
